...
export class FileResource implements Resource {
    ...
    protected async isOutdated(): Promise<boolean> {
        if (!this.state.stat) {
            return true;
        }
        const stat = await this.fileSystem.getFileStat(this.uriString);
        /*
         * if timestamps equal we cannot assume that content has not been changed because of
         * https://nodejs.org/en/docs/guides/working-with-different-filesystems/#timestamp-resolution
         */
        return stat.lastModification >= this.state.stat.lastModification;
    }
